...
class Simulation:
    ...
    def _load_elevators(self, curr_floor: int,
        ...
        temp_arrivals = arrivals_at_floor.copy()
        for person in temp_arrivals:
            person_loaded = False

            for elevator in self.elevators:
                while elevator.fullness() < 1.0 \
                        and elevator.floor == curr_floor and not person_loaded:
                    person_boarding = person
                    arrivals_at_floor.remove(person)
                    elevator.passengers.append(person_boarding)
                    person_loaded = True
                    self.visualizer.show_boarding(person_boarding, elevator)
