...
class SimplePrefixTree(Autocompleter):
    ...
    def _get_sum_weights(self) -> float:
        ...
        if self.value == []:
            ...
            for subtree in self.subtrees:
                sum += subtree._get_sum_weights()

            return sum_weights

    # TODO Check if working
    def _get_ave_weights(self) -> Tuple[float, int]:
        """Helper method to traverse tree and return sum of all leaf weights
        ...
        if self.value == []:
            ...
            return tuple(self.weight, 1)

        else:
            sum_weights = 0.0
            num_leaves = 0
            for subtree in self.subtrees:
                t = subtree._get_sum_weights()[0]
                sum_weights += t[0]
                num_leaves += t[1]

            return tuple(sum_weights, num_leaves)

    # TODO Test not testing rn
    # Adds leaves only
    def __len__(self) -> int:
    ...
    def insert(self, value: Any, weight: float, prefix: List):
        ...
        'carol'
        >>> t.subtrees[0].subtrees[0].subtrees[0].subtrees[0].subtrees[0].subtrees[0].weight
        4.0

        """
        # Sanitizes input to make all chars lowercase
        value = value.lower()
        # TODO need to sanitize prefix

        # If self.value is the prefix
        if self.value == prefix:
            ...
            if not found_value:
                ...
                self.subtrees[len(self.subtrees)].value = value

        # If self.value is a fragment of prefix
            ...
            if not found_fragment:
                self.subtrees.append(SimplePrefixTree(self.weight_type))
                self.subtrees[len(self.subtrees)].value = prefix[0: len(self.value) + 1]
                self.subtrees[len(self.subtrees)].insert(weight, value, prefix)

        else:
            # Add prefix fragments
            self.subtrees.append(SimplePrefixTree(self.weight_type))
            self.subtrees[len(self.subtrees)].value = prefix[0: len(self.value) + 1]
            self.subtrees[len(self.subtrees)].insert(weight, value, prefix)
    ...
    def _add_prefix_steps(self, depth: int, value: Any, prefix: List[Any],
                          weight: float) -> None:
        """Helper method to add prefix in steps
            First call always has depth zero"""
        # TODO might add duplicate value rn
        if depth == len(prefix):
            self.subtrees.append(SimplePrefixTree(self.weight_type))
            ...
            found_prefix = False
            for subtree in self.subtrees:
                if subtree.value == prefix[0: depth + 1]:
                    subtree._add_prefix_steps(depth + 1, value, prefix, weight)
                    found_prefix = True
                    break

            if not found_prefix:
                self.subtrees.append(SimplePrefixTree(self.weight_type))
                self.subtrees[len(self.subtrees) - 1].value = prefix[0:depth + 1]
                self.subtrees[len(self.subtrees) - 1]\
                    ._add_prefix_steps(depth + 1, value, prefix, weight)

    # TODO not working rn
    def _contains_value(self, subtrees: List[SimplePrefixTree]) -> Tuple[Optional[int], bool]:
        """Helper method that searches subtree list for value == to item.
           - Returns a tuple with the index of found value,
           and bool based on if value is found
           - Return None as index if not found
        """
        if subtrees == []:
            return None, False

        for i in range(len(subtrees)):
            if type(subtrees[i].value) != list:
                return i, True
            else:
                return subtrees[i]._contains_value(subtrees[i].subtrees)
