...
class Player:
    '''A superclass for players within the NumberGame class
    ...
    def move(self, current: int, min_step: int, max_step: int, goal: int) -> int:
        '''Abstract class updates the NumberGame's current total with
        the player's move value

        Movement values will be decided by subclasses
        '''
...
class RandomPlayer(Player):
    def move(self, current: int, min_step: int, max_step: int, goal: int) -> int:
        '''Overwrites the move method from the superclass and returns a
         random int between min_step and max_step
         '''
        return current + random.randint(min_step, max_step + 1)


class UserPlayer(Player):
    def move(self, current: int, min_step: int, max_step: int, goal: int) -> int:
        '''Adds the input value to count if min_step <= input <= max_step
        '''

        within_bounds = False
        while not within_bounds:
            try:
                move_value = int(input("Move value: "))
                if move_value <= max_step and move_value >= min_step:
                    within_bounds = True
                    return current + move_value
                else:
                    print("Boi. Enter a number within the bounds")

            except:
                print("Boi. Enter an actual number")
