...
class Tree:
    ...
    def __init__(self, root: Optional[Any], subtrees: List[Tree]) -> None:
        ...
        self.size = self._count_subtree_size()

    def _count_subtree_size(self) -> int:
        if self.is_empty():
            return 0
            ...
            for subtree in self._subtrees:
                sum += subtree._count_subtree_size()
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty():
            ...
            sum_branches, num_internals = self._branching_helper()
            return sum_branches / num_internals

    def _branching_helper(self) -> Tuple[float, float]:
        """Helper method for branching_factor"""
        if self._subtrees == []:
            return 0.0, 0.0

        else:
            sum_branches = len(self._subtrees)
            num_internals = 1.0
            for subtree in self._subtrees:
                t = subtree._branching_helper()
                sum_branches += t[0]
                num_internals += t[1]
    ...
    def items_at_depth(self, d: int) -> List:
        ...
        >>> t1.items_at_depth(2)
        []
        >>> t2 = Tree(4, [Tree(5, [Tree(7, [])]), Tree(6, [])])
        >>> t2.items_at_depth(3)
        [7]
        """
        if self.is_empty():
            ...
            depth_values = []
            for subtree in self._subtrees:
                depth_values.extend(subtree.items_at_depth(d - 1))
    ...
    def insert(self, item: Any) -> None:
        ...
        if self.is_empty():
            ...
            if choice == 3:
                ...
                chosen_tree = self._subtrees[
                    random.randint(0, len(self._subtrees) - 1)]

                if chosen_tree._subtrees == []:
                    chosen_tree._subtrees.append(Tree(item, []))

                else:
                    chosen_tree._subtrees[0].insert(item)
