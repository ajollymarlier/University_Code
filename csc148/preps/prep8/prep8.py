...
class Tree:
    ...
    def height(self: Tree) -> int:
        ...
        >>> t1.height()
        1
        >>> t2 = Tree(1, [Tree(-2, [Tree(-4, [])]), Tree(10, []), Tree(-30, [])])
        >>> t2.height()
        3
        """
        # if self.is_empty():
    ...
    def __contains__(self, item: Any) -> bool:
        ...
        >>> t = Tree(-30, [Tree(-2, []), Tree(-3, [Tree(-3, [Tree(-3, [])])]), Tree(-3, [])])
        ...
        if self.is_empty():
            ...
            tree_contains = self._root == item

            for subtree in self._subtrees:
                child_val = subtree.__contains__(item)

                if child_val or subtree._root == item:
                    tree_contains = True

            return tree_contains
